...
def main():
    dirpath = os.path.join(os.path.dirname(__file__), "file\\globalalignment")
    filepaths = glob(os.path.join(dirpath, "data*.txt"))

    for path in filepaths:
        reward, mismatch, indel, seq1, seq2= ReadTest_GlobalAlignment(path)
        score, final_seq1, final_seq2 = GlobalAlignment(reward, mismatch, indel, seq1, seq2)
...
def ReadTest_GlobalAlignment(filepath) -> tuple[int, int, int, str, str]:
...
def GlobalAlignment(char_match, char_mismatch, gap, seq1: str, seq2: str) -> tuple[int, str, str]:
    """
